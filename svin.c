...
void _svin_init()
{
    ...
    lfsr = 0xACE1u; //replace with a real random value later
    //-------------- setup VDP2 -------------------
    ...
    for (int i=0; i<15; i++)
    {
        vdp1_cmdt_normal_sprite_set(&cmdts[_SVIN_VDP1_ORDER_SPRITE_BLOCK_A_INDEX(i)]);
        vdp1_cmdt_param_draw_mode_set(&cmdts[_SVIN_VDP1_ORDER_SPRITE_BLOCK_A_INDEX(i)], sprite_draw_mode);
        vdp1_cmdt_normal_sprite_set(&cmdts[_SVIN_VDP1_ORDER_SPRITE_BLOCK_B_INDEX(i)]);
        vdp1_cmdt_param_draw_mode_set(&cmdts[_SVIN_VDP1_ORDER_SPRITE_BLOCK_B_INDEX(i)], sprite_draw_mode);
    }
...
#define VDP1_FBCR_DIE (0x0008)
    ...
    vdp1_cmdt_param_color_mode4_set(cmdt_sprite, dummy_bank);
    vdp1_cmdt_param_color_bank_set(cmdt_sprite, dummy_bank);
    //vdp1_cmdt_jump_assign(cmdt_sprite, _SVIN_VDP1_ORDER_SPRITE_BLOCK_A_INDEX(0) * 4);//skipping A2 and A3
    cmdt_sprite->cmd_pmod |= 0x08C0; //enabling ECD and SPD manually for now
    ...
    vdp1_cmdt_param_color_mode4_set(cmdt_sprite, dummy_bank);
    vdp1_cmdt_param_color_bank_set(cmdt_sprite, dummy_bank);
    //vdp1_cmdt_jump_assign(cmdt_sprite, _SVIN_VDP1_ORDER_SPRITE_BLOCK_B_INDEX(0) * 4);//skipping B2 and B3
    cmdt_sprite->cmd_pmod |= 0x08C0; //enabling ECD and SPD manually for now

    for (int i=0;i<15;i++)
    {
        cmdt_sprite = &cmdts[_SVIN_VDP1_ORDER_SPRITE_BLOCK_A_INDEX(i)];
        cmdt_sprite->cmd_xa = 164+96*(i%4);
        cmdt_sprite->cmd_ya = 16+48*(i/4);
        cmdt_sprite->cmd_size = 0x0C30;
        cmdt_sprite->cmd_srca = ((int)vdp1_vram_partitions.texture_base - VDP1_VRAM(0) + _SVIN_SCREEN_WIDTH*_SVIN_SCREEN_HEIGHT + i*96*48 ) / 8;
        vdp1_cmdt_param_color_mode4_set(cmdt_sprite, dummy_bank);
        vdp1_cmdt_param_color_bank_set(cmdt_sprite, dummy_bank);        
        //vdp1_cmdt_jump_assign(cmdt_sprite, _SVIN_VDP1_ORDER_DRAW_END_B_INDEX * 4);//skipping B2 and B3
        cmdt_sprite->cmd_pmod |= 0x08C0; //enabling ECD and SPD manually for now

        cmdt_sprite = &cmdts[_SVIN_VDP1_ORDER_SPRITE_BLOCK_B_INDEX(i)];
        cmdt_sprite->cmd_xa = 164+96*(i%4);
        cmdt_sprite->cmd_ya = 16+48*(i/4);
        cmdt_sprite->cmd_size = 0x0C30;
        cmdt_sprite->cmd_srca = ((int)vdp1_vram_partitions.texture_base - VDP1_VRAM(0) + _SVIN_SCREEN_WIDTH*_SVIN_SCREEN_HEIGHT + (i+15)*96*48 ) / 8;
        vdp1_cmdt_param_color_mode4_set(cmdt_sprite, dummy_bank);
        vdp1_cmdt_param_color_bank_set(cmdt_sprite, dummy_bank);
        //vdp1_cmdt_jump_assign(cmdt_sprite, _SVIN_VDP1_ORDER_DRAW_END_B_INDEX * 4);//skipping B2 and B3
        cmdt_sprite->cmd_pmod |= 0x08C0; //enabling ECD and SPD manually for now
